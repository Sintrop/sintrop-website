      ...
      <DialogContent className="flex flex-col p-3">
        ...
        <div className="flex flex-wrap gap-3">
          {appData?.links.map((link) => (
            <LinkItem href={link.href} title={link.title} key={link?.href} />
          ))}
        </div>
...
function ContentLink({ appData, t }: ContentLinkProps) {
  return (
    <button className="flex text-start rounded-md bg-green-2 gap-4 pr-5 w-full md:w-[48%] hover:cursor-pointer">
      ...
      <div className="flex flex-col gap-1 pt-2 max-w-[60%]">
        <h4 className="leading-4.5">{t(appData?.title)}</h4>
        <div className="flex flex-wrap gap-3">
          {appData?.mainnet ? (
